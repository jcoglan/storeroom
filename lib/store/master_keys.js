...
class MasterKeys {
  ...
  hashPathname(pathname) {
    let hmac   = crypto.createHmac('sha256', this._hashKey),
  ...
  static create(options) {
    return options.adapter.read(FILENAME).then((keys) => {
      if (keys)
  ...
  static generate(options) {
    ...
    let fileHead = JSON.stringify(metadata.fileHeader);

    let work     = Math.pow(2, WORK_FACTOR),
  ...
  static parse(options, keys) {
    ...
    let offset1 =           consts.HASH_KEY_SIZE,
