...
var MasterKeys = function(keys, options) {
  ...
  this._cipher   = new Cipher(keys.slice(1), {input: 'binary', format: 'binary'});
};
...
MasterKeys.create = function(options) {
  var adapter  = options.adapter,
      password = new Cipher(options.password, {input: 'binary', salt: SALT, work: WORK});
  ...
  return adapter.read(FILENAME).then(function(keys) {
    ...
    return adapter.write(FILENAME, block).then(function() { return newKeys });
...
MasterKeys.parse = function(keys, password) {
  keys = new Buffer(password.decrypt(keys), 'binary');
