...
Store.prototype.put = function(pathname, value) {
  return this.update(pathname, function() { return value });
};

Store.prototype.update = function(pathname, fn) {
  ...
  return this._getKeys().then(function(masterKeys) {
    ...
    return self._withBuckets(names, masterKeys, function(buckets) {
      ...
      var doc = buckets[docBucket].get(pathname);
      buckets[docBucket].put(pathname, fn(doc));
    });
  });
};
