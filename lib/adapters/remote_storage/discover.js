...
var rsLinkFromResponse = function(response, resource) {
  ...
  if (rsLink.template) {
    if (rsLink.rel === 'lrdd')
      return request('GET', rsLink.template.replace('{uri}', resource)).then(rsLinkFromResponse);
