...
DropboxAdapter.prototype.read = function(name) {
  ...
  return request('POST', 'https://content.dropboxapi.com/2/files/download', {}, {
    ...
    if (status === 409) {
      ...
      if (props.error && props.error.path && props.error.path['.tag'] === 'not_found')
        return null;
    }
...
DropboxAdapter.prototype.write = function(name, data) {
  ...
  return request('POST', 'https://content.dropboxapi.com/2/files/upload', data, {
    'Authorization':   'Bearer ' + this._token,
    ...
    if (status === 409) {
      props = JSON.parse(body);
      if (props.error && props.error.reason && props.error.reason['.tag'] === 'conflict')
...
DropboxAdapter.prototype._delete = function(name) {
  ...
  return request('POST', 'https://api.dropboxapi.com/2/files/delete', data, {
    'Authorization': 'Bearer ' + this._token,
    ...
    if (status === 409) {
      props = JSON.parse(body);
      if (props.error && props.error.path_lookup && props.error.path_lookup['.tag'] === 'not_found')
        return;
    }
